...
# 
#     # {'food_id': '12100664', 'food_name': 'Kimchi', 'brand_name': 'Kimchi Pride & Foods', 'food_type': 'Brand', 'food_url': 'https://foods.fatsecret.com/calories-nutrition/kimchi-pride-and-foods/kimchi', 'servings': {'serving': [{'serving_id': '11487081', 'serving_description': '1 oz', 'serving_url': 'https://foods.fatsecret.com/calories-nutrition/kimchi-pride-and-foods/kimchi', 'metric_serving_amount': '28.000', 'metric_serving_unit': 'g', 'number_of_units': '1.000', 'measurement_description': 'serving', 'calories': '10', 'carbohydrate': '2.00', 'protein': '1.00', 'fat': '0', 'saturated_fat': '0', 'polyunsaturated_fat': '0', 'monounsaturated_fat': '0', 'trans_fat': '0', 'cholesterol': '0', 'sodium': '160', 'potassium': '0', 'fiber': '1.0', 'sugar': '0'}]}}
#     # 음식 이름만 추출
#     food_items = result['foods_search']['results']['food']
# 
#     food_names = [f['food_name'] for f in food_items]
#     food_ids = [f['food_id'] for f in food_items]
# 
...
#     """
#     FatSecret food.get -> 영양소 수집
#     + ingredients 비면 recipes.search.v2 -> recipe.get.v2 로 보강
#     """
#     try:
#         access_token = get_token()
#         headers = {"Authorization": f"Bearer {access_token}"}
...
# 
#         # 2) ingredients 시도 1: (일부 계정/아이템에선 올 수 있어 우선 체크)
#         raw_ing = (food.get("ingredients") or "").strip()
#         ingredients_list: list[str] = [
#             s.strip() for s in raw_ing.replace(";", ",").split(",") if s.strip()
#         ]
# 
#         # 3) ingredients 시도 2: 없으면 레시피 API 경유
#         if not ingredients_list:
#             # a) 레시피 검색 (food_name 기준)
#             rec_search = _fs_post(headers, {
#                 "method": "recipes.search.v2",
#                 "format": "json",
#                 "search_expression": food_name,
#                 "max_results": 3,   # 소수 상위만
#             })
#             recipes = rec_search.get("recipes", {}).get("recipe", [])
#             if isinstance(recipes, dict):
#                 recipes = [recipes]
# 
#             # 간단 스코어: 제목 포함 여부/길이 차이 등으로 정렬
#             def score(r: dict[str, Any]) -> tuple[int, int]:
#                 name = (r.get("recipe_name") or "").lower()
#                 fn = (food_name or "").lower()
#                 contains = 0 if fn and (fn in name or name in fn) else 1  # 포함되면 0(우선)
#                 diff = abs(len(name) - len(fn))
#                 return (contains, diff)
# 
#             recipes.sort(key=score)
# 
#             # b) 최상위 레시피 상세로부터 재료 수집
#             if recipes:
#                 rid = recipes[0].get("recipe_id")
#                 if rid:
#                     rec_detail = _fs_post(headers, {
#                         "method": "recipe.get.v2",
#                         "format": "json",
#                         "recipe_id": rid,
#                     })
#                     ingr_block = rec_detail.get("recipe", {}).get("ingredients", {}).get("ingredient", [])
#                     if isinstance(ingr_block, dict):
#                         ingr_block = [ingr_block]
#                     extracted = []
#                     for it in ingr_block:
#                         # ingredient_description(수량/형태 포함) 우선, 없으면 food_name
...
# 
#         # 4) 그래도 비면 최소한의 fallback (Generic은 자기 이름을 재료로 본다)
#         if not ingredients_list:
#             if food_type == "Generic" and food_name:
#                 ingredients_list = [food_name]
#             else:
#                 ingredients_list = []  # 정말 없으면 빈 리스트 유지
# 
#         return {
